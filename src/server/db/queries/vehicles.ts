...
export const getUserVehicles = async (id: string) => Query("SELECT * FROM userVehicles WHERE userid = ?", [id]);
...
export const getVehicleInfo = async (make: string, model: string, year: number) => Query("SELECT * FROM vehicleData WHERE make = ? AND model = ? AND year = ?", [make, model, year])
...
export const getMaintenanceInfo = async (id:string) => Query("SELECT * FROM maintenanceInfo WHERE vehicleId = ?", [id]);
...
export const getEngineLightInfo = async (dtc: string) => Query("SELECT * FROM maintenanceInfo WHERE dtc = ?", [dtc]);
...
export const updateUserVehicleDate = async (id: string, column: string, value: string) => Query("CALL spUpdateUserVehicle(?, ?, ?)", [id, column, value]);

//The query above specifically can only do timestamps, so this is the query you use to update the mileage on a user vehicle
export const updateMileage = async (mileage: number, id: string) => Query("UPDATE userVehicles SET milage = ? WHERE id = ?", [mileage, id]);
...
export const addNewVehicle = async (vin: string, userid: string) => Query("INSERT INTO userVehicles(vin, userid) VALUES(?, ?)", [vin, userid]);
